...
class AdminController extends Controller {
    ...
    protected function delete ( $model , $where = array() , $msg = array( 'success'=>'删除成功！', 'error'=>'删除失败！')) {
        ...
        M()->startTrans();
        $res =  M('Member')->where($where)->delete();

        if ( !M('Auth_group_access')->where($where)->find() ) {
            $res2= true;
        }else{
              $res2 = M('Auth_group_access')->where($where)->delete();
        }
      
      
        $map['id'] = array('in',$where['uid'][1]);

        $res3 = M('Ucenter_member')->where($map)->delete();      

        //  echo M('Auth_group_access')->getLastSql();
        // exit();
        if ($res && $res2 && $res3) {
            M()->commit();
            $this->success('删除成功');
    ...
    /**
     * 无重复合并
     * [array_values_merge description]
