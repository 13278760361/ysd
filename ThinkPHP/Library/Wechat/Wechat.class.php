...
class Wechat
{
    ...
    //图文内容的图片上传接口
    const MEDIA_CONTENT_IMG_UPLOAD_URL = '/media/uploadimg?';
	...
	//获取消息模板列表
    const GET_ALLPRIVATE_TEMPLATE = '/template/get_all_private_template?';
	///多客服相关地址
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	//下载多媒体文件
    public function saveMedia($media_id, $dirname, $is_video=false){
    	if (!$this->access_token && !$this->checkAuth()) return false;
		//原先的上传多媒体文件接口使用 self::UPLOAD_MEDIA_URL 前缀
		//如果要获取的素材是视频文件时，不能使用https协议，必须更换成http协议
		$url_prefix = $is_video?str_replace('https','http',self::API_URL_PREFIX):self::API_URL_PREFIX;
		$result = $this->http_get($url_prefix.self::MEDIA_GET_URL.'access_token='.$this->access_token.'&media_id='.$media_id, true);

		$media = array_merge(array('mediaBody' => $result['curl_exec']), $result['curl_getinfo']);
     ...
     * @return boolean|array
     */
    public function uploadContentMedia($data, $type,$is_video=false,$video_info=array()){
        if (!$this->access_token && !$this->checkAuth()) return false;
        //#TODO 暂不确定此接口是否需要让视频文件走http协议
        //如果要获取的素材是视频文件时，不能使用https协议，必须更换成http协议
        //$url_prefix = $is_video?str_replace('https','http',self::API_URL_PREFIX):self::API_URL_PREFIX;
        //当上传视频文件时，附加视频文件信息
        if ($is_video) $data['description'] = self::json_encode($video_info);
        $result = $this->http_post(self::API_URL_PREFIX.self::MEDIA_CONTENT_IMG_UPLOAD_URL.'access_token='.$this->access_token.'&type='.$type,$data,true); 
        if ($result)
        {
            $json = json_decode($result,true);
            // dump($json);exit();
	...
	public function deleteGroup($groupid){
		...
		$result = $this->http_post(self::API_URL_PREFIX.self::GROUP_DEL_URL.'access_token='.$this->access_token,self::json_encode($data));
		if ($result)
		{
			$json = json_decode($result,true);
			if (!$json || $json['errcode'] != 0) {
	...
	public function getAllPrivateTemplate(){ 
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_URL_PREFIX.self::GET_ALLPRIVATE_TEMPLATE.'access_token='.$this->access_token,self::json_encode($data));
		if($result){
			$json = json_decode($result,true);
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
